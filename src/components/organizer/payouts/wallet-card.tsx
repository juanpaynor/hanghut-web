...
import { Wallet, Plus, AlertTriangle, ExternalLink, Shield } from 'lucide-react'
...
export function WalletCard({ xenditAccountId, receivable, kycStatus, availableBalance }: WalletCardProps) {
    const [topUpOpen, setTopUpOpen] = useState(false)
    const [amount, setAmount] = useState('')
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState('')

    const isWalletReady = !!xenditAccountId && kycStatus === 'verified'
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <p className="text-sm text-muted-foreground">Xendit Wallet</p>
                            {kycStatus === 'verified' ? (
                        ...
                        {receivable > 0 && (
                            <div className="flex items-center gap-1.5 mt-2">
                                ...
                                <p className="text-xs text-amber-600 font-medium">
                                    ₱{receivable.toLocaleString()} owed to platform (from refunds)
                                </p>
                            </div>
                        )}
                    </div>
                    <div className="flex flex-col items-end gap-2">
                        <Wallet className="h-10 w-10 text-purple-500 opacity-80" />
                        {isWalletReady && (
