...
// The rendering half of the hosted ticket page, split out so /t/[token] (ISR,
// the real buyer-facing page) and /t/[token]/preview (dynamic, organizer-only)
// render from ONE source and can never drift apart.
//
// The split exists because reading searchParams in a route opts it out of static
// rendering. Putting the preview override on the live page would have made all
// 563+ tickets hit the DB on every view, undoing the ISR the page was written
// for (see the cookieless-client note in page.tsx).

export type TicketTemplate = 'classic' | 'boarding' | 'minimal' | 'stub'
...
export function TicketOrderView({
    order,
    templateOverride,
}: {
    order: TicketOrder
    /** Preview-only override. Never read from or written to the database. */
    templateOverride?: TicketTemplate
}) {
    ...
    // templateOverride is preview-only and never persisted; with it unset this
    // resolves exactly as before, so live tickets render byte-identically.
    const template: TicketTemplate = templateOverride ?? design.template ?? 'classic'
    ...
    function TicketCard({ t }: { t: OrderTicket }) {
        ...
        if (template === 'stub') {
            // A self-contained concert stub. The other templates carry the event
            // title only in the page header, so a buyer who screenshots just the
            // card arrives at the door with a bare QR and no context — this one
            // repeats the event, date and venue inside the card so the screenshot
            // alone is enough. Tier colour drives the head band.
            const band = tColor || accent || '#4E47DC'
            return (
                <div className="overflow-hidden rounded-2xl border bg-background shadow-sm">
                    {/* Tier band */}
                    <div
                    ...
                    >
                        ...
                        <span className="shrink-0 rounded-full bg-white/20 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
                            {badge.label}
                        </span>
                    </div>

                    {/* Event context — what makes the stub stand alone */}
                    <div className="space-y-1 px-5 pt-4">
                        <p className="font-headline text-lg font-bold leading-tight">{event.title}</p>
                        {eventDate && (
                    ...
                    {/* Seat gets its own row — it is the thing staff ask for */}
                    {seat && (
                        <div className="mx-5 mt-4 flex items-center gap-2 rounded-lg bg-muted/60 px-3 py-2">
                    ...
                    <div className="relative mt-5 flex items-center">
                        <span className="absolute -left-2 h-4 w-4 rounded-full bg-muted/60" />
                        <div className="w-full border-t-2 border-dashed border-muted-foreground/30" />
                        <span className="absolute -right-2 h-4 w-4 rounded-full bg-muted/60" />
                    </div>

                    {/* QR stub */}
                    <div className="flex flex-col items-center gap-2.5 px-5 pb-6 pt-5">
                        <div className={isUsed ? 'opacity-40 grayscale' : ''}>
